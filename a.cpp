...
for (;;)
{
	total++;
    for (int i = 0; i < n; i++) {
        arr1[i] = arr2[i] = rand() % 256;
    }

	int normal1 = crc16_form2((UINT8*)arr1,n,0);
	int abnormal1 = crc16_form3((UINT8*)arr1,n,0);
	...
	while (a0==a1 || a0==a2 || a0==a3 || a1==a2 || a1==a3 || a2==a3)
	{
		a1 = rand() % n;
		a2 = rand() % n;
		a3 = rand() % n;
...
//if (arr1[b1]==arr2[b1]) {printf("total=%d %d %d %d %d \r\n",total, b1, ss, ii, arr1[b1]);printArr();}
//else {printf("!!!%d %d %d %d \r\n",b1, ss, ii, arr1[b1]);printArr();}
	int normal2 = crc16_form2((UINT8*)arr2,n,0);
	int abnormal2 = crc16_form3((UINT8*)arr2,n,0);
	
	if (normal1==normal2)
	{
		norm++;
		printf("Standard CRC16 collision!!!, %d/%d/%d/%d %d %d/%d/%d\r\n", a0,a1,a2,a3, ii1,norm, ab, total);
		printArr();
	}
	
	if (abnormal1==abnormal2)
	{
		ab++;
		if (ab%100000==0)
		{
			printf("Private CRC16 collision %d/%d/%d/%d %d %d/%d/%d\r\n", a0,a1,a2,a3, ii1,norm, ab, total);
			//printArr();
		}
	}
}
